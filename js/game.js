	...
	$(document).ready(function(){
            ...
            if ($('#ic-image').prop('files')[0]) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $('.content-wrapper .content.upload-form').fadeOut();
                        $('.content-wrapper #upload-holder').fadeIn();
                        $('#commander-image img').attr('src',response);
        ...
        $(document).on('click', '#commander-overlay .tempit', function(e) {
            if(!$('.step3').hasClass('on')) {      
                ...
                var percX = relX / $(this).width() * 100;

                var relY = e.pageY - parentOffset.top;
                var percY = relY / $(this).height() * 100;
                ...
                $('#commander-overlay').append('<div class="placeit" style="left: '+percX+'%; top:'+percY+'%;"><div class="checkmark"><div class="check"></div></div></div>');
            }
        });
            ...
            if(!$('.marker.commander').length) {
                ...
                /*if(user_id < 1) {
                    ...
                    if($('.wrongmarker.right').length || $('.wrongmarker.pulse').length) {
                        ...
                        var parentOffset = $('#inspector-image').offset();
                        var relX = e.pageX - parentOffset.left;
                        var percX = relX / $(this).width() * 100;
                        ...
                        $('#inspector-image').append('<div class="wrongmarker pulse" style="left: '+percX+'%; top:'+percY+'%;"></div>');
                        setTimeout(function() {
        ...
        $(document).on('click', '.marker.inspector', function(e) {
            ...
            var relY = e.pageY - parentOffset.top;
            var percY = relY / $(this).height() * 100;*/
            $('.marker.inspector').addClass('wrongmarker right').html('<div class="check"></div>');
            //$('#inspector-image').append('<div class="wrongmarker right" style="left: '+percX+'%; top:'+percY+'%;"><div class="check"></div></div>');
