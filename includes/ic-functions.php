...
function wp_snippet_disable_rest_api( $access ) {
     return new WP_Error( 'rest_disabled', __('Our REST API has been disabled.'), array( 'status' => rest_authorization_required_code()));
}
/*
 *
 * Disable Emojis
...
function eye_spy_it() {
    ...
    if($ready) {
        ...
        echo '<div class="content-wrapper flex-center">';
            echo '<div class="content">'; //had 'full'
                echo '<div id="inspector-image" data-c="'.get_post_meta($post_id,'ic_commander',true).'">';
                echo '<img src="'.$url[0].'" />';
        ...
        if($commander == $current_user_id) {
            echo '<div class="content-wrapper flex-center">';
                echo '<div class="content upload-form">';
                    echo upload_image_form($post_id);
                echo '</div>';
                echo '<div class="content" id="upload-holder">'; //was id="commander-content"
                    echo '<div id="commander-image">';
                        echo '<img src="" />';
        ...
        echo '<div class="login-form">';
            echo '<div class="wrap">';
                ...
                echo '<div class="step3">';
                    echo '<h4>Just one last step.<br>What&rsquo;s your name?</h4>';
...
function end_ic_round(){
    ...
    if(!$calculating) {
        ...
        update_post_meta($post_id,'waiting_on_commander',$time + Game_Timer);
        update_post_meta($post_id, 'ic_start_time', $time);
        wp_mail('greggoryhogan@gmail.com','Someone found it','They did!');
        delete_eyespy_attachments_function();
        $page_version_hash = wp_hash($time);
        $page_version = substr($page_version_hash, 0, 8);
        delete_post_meta( $post_id, 'force_refresh_current_page_version' );
        delete_post_meta($post_id,'calculating');
