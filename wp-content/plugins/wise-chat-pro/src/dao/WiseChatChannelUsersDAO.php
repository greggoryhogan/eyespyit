...
class WiseChatChannelUsersDAO {
	...
	*/
	...
	public function getActiveByUserIdAndChannelId($userId, $channelId) {
		...
		$table = WiseChatInstaller::getChannelUsersTable();
		$sql = sprintf(
			'SELECT * FROM %s WHERE active = "1" AND user_id = %d AND channel_id = %d ORDER BY id DESC LIMIT 1;', $table, intval($userId), intval($channelId)
		);
		$results = $wpdb->get_results($sql);
		if (is_array($results) && count($results) > 0) {
			return $this->populateData($results[0]);
	...
	/**
	 * Returns inactive channel-user by ID.
	 *
	 * @param integer $userId
	 * @param integer $channelId
	 *
	 * @return WiseChatChannelUser|null
	 */
	public function getInactiveByUserIdAndChannelId($userId, $channelId) {
		global $wpdb;

		$table = WiseChatInstaller::getChannelUsersTable();
		$sql = sprintf(
			'SELECT * FROM %s WHERE active = "0" AND user_id = %d AND channel_id = %d ORDER BY id DESC LIMIT 1;', $table, intval($userId), intval($channelId)
		);
		$results = $wpdb->get_results($sql);
		if (is_array($results) && count($results) > 0) {
			return $this->populateData($results[0]);
