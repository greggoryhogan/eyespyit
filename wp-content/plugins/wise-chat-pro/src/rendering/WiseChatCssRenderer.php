...
class WiseChatCssRenderer {
	...
	*/
	public function getCssDefinition($containerId) {
		...
		$this->addDefinition('.wcOperationalSection', 'background_color_input', 'background-color');
		if ($this->options->isOptionNotEmpty('background_color_input')) {
			$this->addRawDefinition('.wcOperationalSection', 'background', 'none');
		}
		...
		if ($sidebarMode) {
			if ($this->options->getOption('chat_width') != '100%') {
				$this->addLengthDefinition('.wcMessages', 'chat_width', 'width', true);
				$this->addLengthDefinition('.wcWindowContent', 'chat_width', 'width', true);
			}
		} else {
		...
		if (!$sidebarMode) {
			...
			$this->addLengthDefinition('.wcUsersList', 'users_list_width', 'width');
			$this->addLengthDefinition('.wcUserListTitle', 'users_list_width', 'width');
		}
