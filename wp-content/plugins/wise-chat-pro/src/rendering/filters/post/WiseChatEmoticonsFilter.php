...
class WiseChatEmoticonsFilter {
    ...
    public function __construct() {
		...
		$this->emoticonsDAO = WiseChatContainer::get('dao/WiseChatEmoticonsDAO');
    }
	...
	*/
	public function filter($text, $emoticonsSet) {
		if ($this->replacementArrays === null) {
			if ($this->options->isOptionEnabled('custom_emoticons_enabled', false)) {
				$this->replacementArrays = $this->prepareReplacementArraysOfCustomEmoticons();
			} else {
	...
	private function prepareReplacementArrays($emoticonsSet) {
		...
		foreach ($emoticons as $emoticon) {
			...
			$tagToEncrypt = array_key_exists($emoticon, $aliases) ? $aliases[$emoticon][0] : '<'.$emoticon.'>';
			$tagEncrypted = base64_encode((WiseChatCrypt::encrypt(gzcompress($tagToEncrypt))));
			$imgTag = sprintf(
				"<img src='%s' alt='%s' class='wcEmoticon' data-tag='%s' data-type='emoticon' />",
				$filePath, htmlspecialchars($emoticon, ENT_QUOTES, 'UTF-8'), $tagEncrypted
			);
	...
	private function prepareReplacementArraysOfCustomEmoticons() {
		$searchArray = array();
		$replaceArray = array();

		foreach ($this->emoticonsDAO->getAll() as $emoticon) {
			$id = 'emoticon-'.$emoticon->getId();

			$url = '';
			if (strlen($this->options->getOption('custom_emoticons_emoticon_width', '')) > 0) {
				$data = wp_get_attachment_image_src($emoticon->getAttachmentId(), $this->options->getOption('custom_emoticons_emoticon_width', ''));
				if (is_array($data)) {
					$url = $data[0];
				} else {
					$url = wp_get_attachment_url($emoticon->getAttachmentId());
				}
			} else {
				$url = wp_get_attachment_url($emoticon->getAttachmentId());
			}
			...
			$tagToEncrypt = strlen($emoticon->getAlias()) > 0 ? $emoticon->getAlias() : '<'.$id.'>';
			$tagEncrypted = base64_encode((WiseChatCrypt::encrypt(gzcompress($tagToEncrypt))));
			$imgTag = sprintf(
				"<img src='%s' alt='Emoticon %s' class='wcEmoticon' data-tag='%s' data-type='emoticon' />",
				$url, htmlspecialchars($id, ENT_QUOTES, 'UTF-8'), $tagEncrypted
			);
			$searchArray[] = htmlentities('<'.$id.'>');
			$replaceArray[] = $imgTag;

			if (strlen($emoticon->getAlias()) > 0) {
				$searchArray[] = $emoticon->getAlias();
				$replaceArray[] = $imgTag;
			}
		}
