	...
	public function filter($text) {
		...
		if (preg_match_all(self::REGEXP, $stripedText, $matches)) {
			...
			foreach ($matches[2] as $key => $detectedHashtag) {
				...
				$tagEncrypted = base64_encode((WiseChatCrypt::encrypt(gzcompress('#'.$detectedHashtag))));
				$linkTag = sprintf(
					'<a href="%s" target="_blank" rel="noopener noreferrer nofollow" data-tag="%s" data-type="hashtag">%s</a>',
					$url, $tagEncrypted, '#'.$detectedHashtag
				);
