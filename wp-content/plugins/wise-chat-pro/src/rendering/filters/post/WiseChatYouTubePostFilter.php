...
class WiseChatYouTubePostFilter {
    ...
    const YOU_TUBE_IFRAME_TEMPLATE = '<iframe width="%d" height="%d" class="wcVideoPlayer" src="https://www.youtube.com/embed/%s" data-tag="%s" data-type="yt" frameborder="0" allowfullscreen></iframe>';
		...
		if (preg_match_all(self::SHORTCODE_REGEXP, $text, $matches)) {
			...
			foreach ($matches[0] as $key => $shortCode) {
				...
				if ($youtubeEnabled && strlen($movieId) > 0) {
					$replace = sprintf(self::YOU_TUBE_IFRAME_TEMPLATE, $playerWidth, $playerHeight, $movieId, $tagEncrypted);
					...
					$replace = sprintf('<a href="%s" target="_blank" rel="noopener noreferrer nofollow" data-tag="%s" data-type="ytlink">%s</a>', $url, $tagEncrypted, $linkBody);
