...
class WiseChatPostReversedFilter {
	...
	private $tagTypes = array('link', 'img', 'attachment', 'hashtag', 'ytlink', 'emoticon', 'yt');
	...
	public function getTextCharactersCount($text) {
		...
		foreach ($elements as $element) {
			$count += strlen($element->nodeValue);
		}
	...
	private function filterReverseInternal($text) {
		...
		foreach ($this->tagTypes as $tagName) {
			$attributeQuery[] = '@data-type="'.$tagName.'"';
		}
		$elements = $xpath->query('//*['.implode(' or ', $attributeQuery).']');
		...
		foreach ($elements as $element) {
			...
			$tagEncoded = $element->getAttribute('data-tag');
			if (strlen($tagEncoded) > 0) {
				$tagDecryptedCompressed = WiseChatCrypt::decrypt(base64_decode($tagEncoded));
				$tagDecrypted = @gzuncompress($tagDecryptedCompressed);
